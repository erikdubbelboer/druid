...
public class DimensionsFilter implements Filter
{
  ...
  {
    final ValueGetter[] valueGetters = new ValueGetter[dimensions.length];
    ...
    for (int i = 0; i < dimensions.length; i++) {
      final ColumnSelectorPlus<ValueMatcherColumnSelectorStrategy> selector =
          DimensionHandlerUtils.createColumnSelectorPlus(
              ...
              DefaultDimensionSpec.of(dimensions[i]),
              factory
          );
    ...
    return makeValueMatcher(valueGetters, extractionFn);
  }

  public static ValueMatcher makeValueMatcher(final ValueGetter[] valueGetters, final ExtractionFn extractionFn) {
        ...
        for (int i = 0; i < valueGetters.length; i++) {
          ...
          if (extractionFn != null) {
            for (int j = 0; j < values[i].length; j++) {
              values[i][j] = extractionFn.apply(values[i][j]);
            }
          }
  ...
  public String toString()
  {
    ...
    return String.format("(%s)", IS_JOINER.join(extractionFnDimensions));
  }
}
